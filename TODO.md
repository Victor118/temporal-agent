...
- [ ] Borne de délégation — `AgentChain` est propagée mais jamais vérifiée : ajouter un compteur de profondeur et une détection de cycle (A → B → A) dans `buildChildInput`, bornés par configuration, le refus étant rendu au LLM comme un tool result normal. L'exclusion de soi-même n'existe aujourd'hui que dans le prompt (`buildAgentsDirectory`), pas dans le dispatch
