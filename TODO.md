...
- [ ] Politiques de retry/timeout par tool — aujourd'hui tous les tools partagent une seule `ActivityOptions` (120 s, `MaximumAttempts: 1`) alors que `CallLLM` a la sienne (backoff, 6 tentatives, `NonRetryableErrorTypes`). Porter le même modèle sur les tools, avec des valeurs déclarées par tool (un `claude_code` ou un `video_generate` n'a ni le timeout ni la politique de retry d'un `fs_read`)
- [ ] Borne de délégation — l'auto-délégation est fermée (enum `agent_id` + refus au dispatch), mais A → B → A reste possible : passer `AgentChain` dans `ListToolsInput` pour exclure aussi les ancêtres de l'enum (le cycle devient irreprésentable), et ajouter un compteur de profondeur borné par configuration pour la récursion en arbre (A → B → C → …), le refus étant rendu au LLM comme un tool result normal
- [ ] `delegates_to` par agent — aujourd'hui un agent peut déléguer à tout le catalogue sauf lui-même. Si on veut restreindre, colonne `agents.delegates_to` en globs comme `tools` — mais trancher d'abord la sémantique `NULL` vs `[]` (cf. ci-dessous), car ici un `NULL` mal interprété redonne indirectement accès aux tools d'un autre agent
- [ ] Sémantique `NULL` vs `[]` de `agents.tools` — `NULL` = tous les tools, `[]` = aucun. Deux états voisins aux effets opposés, à figer avant l'UI d'édition d'agents (un formulaire qui envoie `[]` pour « rien coché » ouvrirait ou fermerait tout selon l'interprétation)
