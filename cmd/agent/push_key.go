...
// checkPushKey refuses to start a worker whose push identity it could not use.
//
...
func checkPushKey(path string) error {
	if path == "" {
		return nil
	}
	...
	if err != nil {
		return fmt.Errorf("push key %s: %w", path, err)
	}
	if fi.IsDir() {
		return fmt.Errorf("push key %s is a directory", path)
	}
	// ssh refuses a private key others can read, and says so only at push time.
	if perm := fi.Mode().Perm(); perm&0o077 != 0 {
		return fmt.Errorf("push key %s is mode %#o: ssh refuses a key readable by group or others, it must be 0600", path, perm)
	}
	...
	if out, err := cmd.CombinedOutput(); err != nil {
		...
		if strings.Contains(detail, "incorrect passphrase") || strings.Contains(detail, "load failed") {
			return fmt.Errorf("push key %s cannot be used unattended: it is passphrase-protected", path)
		}
		return fmt.Errorf("push key %s is unusable: %s", path, detail)
	}
	return nil
}
